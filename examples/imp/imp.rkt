(require-python-source "examples/imp/imp-ops.py")
...
(define-metafunction Imp
  ...
  [(var-lookup ((var_1 int_1) ...) var) ,(variable_not_found_exception (term var))])
...
(define-metafunction Imp
  setup-loc : (Com ...) -> (Loc (Com ...))
  [(setup-loc (Com_1 ...)) (() (Com_1 ...))])

(define-reduction-relation impred Imp
  #:domain Program 
  ...
  [--> (Loc ((in-hole P (int_1 + int_2)) Com ...))
       (Loc ((in-hole P ,(integer_add (term int_1) (term int_2))) Com ...))
       "integer-add"]

  [--> (Loc ((in-hole P (int_1 * int_2)) Com ...))
       (Loc ((in-hole P ,(integer_mul (term int_1) (term int_2))) Com ...))
       "integer-mul"]

  [--> (Loc ((in-hole P (bool_1 and bool_2)) Com ...))
       (Loc ((in-hole P ,(boolean_and (term bool_1) (term bool_2))) Com ...))
       "boolean-and"]

  [--> (Loc ((in-hole P (bool_1 or bool_2)) Com ...))
       (Loc ((in-hole P ,(boolean_or (term bool_1) (term bool_2))) Com ...))
       "boolean-or"]

  [--> (Loc ((in-hole P (not bool_1)) Com ...))
       (Loc ((in-hole P ,(boolean_not(term bool_1))) Com ...))
       "boolean-not"]

  [--> (Loc ((in-hole P (int_1 <= int_2)) Com ...))
       (Loc ((in-hole P ,(integer_lteq (term int_1) (term int_2))) Com ...))
       "less-equal"])

(read-from-stdin-and-apply-reduction-relation* impred #:apply-mf setup-loc)
